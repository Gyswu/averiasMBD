...
class Container_6fa38303df extends Nette\DI\Container
{
	protected $tags = [
		'nette.inject' => [
			'application.1' => true,
	...
	{
		$service = new Nette\Application\PresenterFactory(new Nette\Bridges\ApplicationDI\PresenterFactoryCallback($this, 5, '/var/www/nette/app/../temp/cache/nette.application/touch'));
	...
	{
		$service = new Nette\Caching\Storages\FileStorage('/var/www/nette/app/../temp/cache');
		return $service;
	}


	public function createServiceContainer(): Container_6fa38303df
	{
		return $this;
	}
	...
	public function createServiceLatte__latteFactory(): Nette\Bridges\ApplicationLatte\ILatteFactory
	{
		return new class ($this) implements Nette\Bridges\ApplicationLatte\ILatteFactory {
			...
			public function __construct(Container_6fa38303df $container)
			{
	...
	{
		$service = new App\Model\Orm\CambiosMapper(
			$this->getService('dbal.connection'),
			$this->getService('orm.mapperCoordinator'),
			$this->getService('orm.cache')
		);
	...
	{
		$service = new App\Model\Orm\CopiasMapper(
			$this->getService('dbal.connection'),
			$this->getService('orm.mapperCoordinator'),
			$this->getService('orm.cache')
		);
	...
	{
		$service = new App\Model\Orm\MaquinasMapper(
			$this->getService('dbal.connection'),
			$this->getService('orm.mapperCoordinator'),
			$this->getService('orm.cache')
		);
	...
	{
		$service = new App\Model\Orm\ProveedorMapper(
			$this->getService('dbal.connection'),
			$this->getService('orm.mapperCoordinator'),
			$this->getService('orm.cache')
		);
	...
	{
		$service = new App\Model\Orm\CambiosRepository($this->getService('orm.mappers.cambios'), $this->getService('orm.dependencyProvider'));
		$service->setModel($this->getService('orm.model'));
		return $service;
	}
	...
	{
		$service = new App\Model\Orm\MaquinasRepository($this->getService('orm.mappers.maquinas'), $this->getService('orm.dependencyProvider'));
		$service->setModel($this->getService('orm.model'));
		return $service;
	}
	...
	{
		$service = new App\Model\Orm\ProveedorRepository($this->getService('orm.mappers.proveedor'), $this->getService('orm.dependencyProvider'));
		$service->setModel($this->getService('orm.model'));
		return $service;
	}
